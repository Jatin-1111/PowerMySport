...
import { ArrowLeft, User, Briefcase, Calendar, AlertCircle } from "lucide-react";
...
export default function AddExpertPage() {
  ...
  const showAddressField = sessionMode === "IN_PERSON" || sessionMode === "BOTH";
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            <div className="mb-6 flex items-center gap-3 border-b border-slate-100 pb-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">Basic information and contact details.</p>
              </div>
            </div>
            
            <div className="grid gap-5 sm:grid-cols-2">
              <InputField label="Full Name" field="name" placeholder="e.g. Rahul Dravid" required />
              <InputField label="Email Address" field="email" type="email" placeholder="e.g. expert@example.com" required />
              <InputField label="Phone Number" field="phone" placeholder="e.g. 9876543210" required />
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            <div className="mb-6 flex items-center gap-3 border-b border-slate-100 pb-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">Service offerings, fees, and bio.</p>
              </div>
            </div>

            <div className="mb-6 grid gap-5 sm:grid-cols-2">
              <InputField label="Session Fee (₹)" field="sessionFee" type="number" min={0} placeholder="e.g. 1500" required />
              ...
              {showAddressField && (
                <div className="sm:col-span-2 flex flex-col gap-1.5">
                  ...
                  <p className="text-xs text-slate-500">
                    Shown to a client only after they've booked a session — never on the public listing.
                  </p>
                </div>
              )}

              <div className="sm:col-span-2 flex flex-col gap-1.5">
                <label className="text-sm font-medium text-slate-700">Sports</label>
                <Controller
                  name="sports"
                  control={control}
                  render={({ field }) => (
        ...
        <div className="space-y-8">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h3 className="mb-4 text-sm font-bold text-slate-900">Profile Photo</h3>
            <CoachPhotoUpload
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mb-5 text-xs leading-relaxed text-slate-500">
              Set the expert's weekly hours so clients can book right away. Leave this blank to let them set it themselves later.
            </p>
            
            <div className="mb-6 flex flex-col gap-1.5 border-b border-slate-100 pb-6">
              <label className="text-xs font-semibold uppercase text-slate-500">Session length (mins)</label>
              <input
                type="number"
                min={15}
                step={15}
