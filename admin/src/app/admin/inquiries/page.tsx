    ...
    try {
      ...
      if (response.success) {
        if (status === "APPROVED") {
          ...
          toast.success(
            `Inquiry approved! Credentials: ${credentials?.email || "N/A"} / ${credentials?.password || "N/A"}`,
          );
        } else {
      ...
      toast.error(getErrorMessage(error, "Failed to review inquiry"));
    } finally {
      ...
      <div className="space-y-6">
        ...
        <Card className="bg-white">
          <div className="py-10 text-center space-y-3">
            ...
            <Button onClick={loadInquiries} variant="secondary">
              Retry
            </Button>
          </div>
        </Card>
          ...
          {totalPages > 1 && (
            <div className="mt-4 flex flex-col gap-3 rounded-lg border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-center text-sm text-slate-600 sm:text-left">
                ...
                {Math.min(currentPage * PAGE_SIZE, inquiries.length)} of{" "}
