...
export const buildCsv = <T>(rows: T[], columns: CsvColumn<T>[]): string => {
  ...
  const lines = rows.map((row) => columns.map((col) => escapeCsvCell(col.value(row))).join(","));
...
export const exportRowsAsCsv = <T>(filename: string, rows: T[], columns: CsvColumn<T>[]): void => {
