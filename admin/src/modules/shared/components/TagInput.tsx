  ...
  return (
    <div
      className={cn(
        ...
        error
          ...
          : "focus-within:border-power-orange focus-within:ring-power-orange/20 border-slate-200"
      )}
    ...
    >
      {value.map((tag, index) => (
        <span
          key={index}
          className="bg-power-orange/10 text-power-orange flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium"
        >
          {tag}
          <button
            type="button"
            ...
            className="hover:bg-power-orange/20 rounded-full p-0.5 focus:outline-none"
          >
            <X className="h-3 w-3" />
          </button>
        </span>
