...
const toneForType = (type: string): string => {
  if (type.startsWith("PAYMENT_") && type.endsWith("_FAILED"))
    return "bg-red-100 text-red-700";
  if (type.startsWith("REFUND_") && type.endsWith("_FAILED"))
    return "bg-red-100 text-red-700";
  ...
  if (events.length === 0) {
    return (
      <p className="text-sm text-slate-500">
        No events recorded. Bookings created before the audit log was introduced
        have no history.
      </p>
    );
  }
            ...
            {hasMetadata && (
              <>
                <button
                  ...
                  className="mt-1 text-xs font-medium text-power-orange hover:underline"
                >
