...
import { scoreSports, scoreChosenSports } from "../src/modules/find-sport/utils/scorer";
...
const BASE: WizardAnswers = {
  ...
  motorType: "gross",
  visualTracking: "strong",
  eyesight: "sharp",
  agility: "high",
  teamIndividual: 3,
  competitiveResponse: "fired-up",
  focusStyle: "bursts",
  decisionStyle: "react",
  pressureResponse: "thrives",
...
// R1: Chess prodigy — strategic, sustained focus, non-athletic, individual, low budget
{
  const a: WizardAnswers = {
    ...BASE,
    energyType: "endurance",
    agility: "low",
    teamIndividual: 1,
    focusStyle: "sustained",
    decisionStyle: "strategic",
    repetitionTolerance: "high",
    contactComfort: "avoids",
    environment: "indoor",
    weeklyHours: "1-3",
    budget: "under-3k",
    pressureResponse: "manages",
    visualTracking: "weak",
  };
...
// R2: Badminton athlete — explosive, high agility, sharp vision, strong tracking
{
  ...
  test(
    "R",
    "R2 Badminton athlete → Badminton in top 3",
    inTop3(a, "Badminton"),
    `got: ${top.join(", ")}`
  );
}
...
// R3: Swimmer — endurance, water-comfortable, repetition-tolerant, individual
{
  ...
  test(
    "R",
    "R3 Swimmer profile → Swimming in top 3",
    inTop3(a, "Swimming"),
    `got: ${top.join(", ")}`
  );
}
...
// R4: Team player — loves team (5), explosive, outdoor
{
  ...
  test(
    "R",
    "R4 Team player → team sport in top 3",
    top.some((s) => teamSports.includes(s)),
    `got: ${top.join(", ")}`
  );
}
...
// R5: Basketball athlete — tall, explosive, team-oriented, indoor
{
  ...
  test(
    "R",
    "R5 Tall explosive team player → Basketball in top 3",
    inTop3(a, "Basketball"),
    `got: ${top.join(", ")}`
  );
}
...
// R6: Contact-tolerant explosive team athlete — loves contact, outdoor, moderate build
{
  ...
  test(
    "R",
    "R6 Contact-tolerant team athlete → Football or Hockey in top 3",
...
// R7: Hockey specialist — outdoor, explosive, visual tracking, moderate team
{
  ...
  test(
    "R",
    "R7 Hockey specialist → Hockey in top 3",
    inTop3(a, "Hockey"),
    `got: ${top.join(", ")}`
  );
}
...
// R8: Table Tennis specialist — explosive, sharp vision, under-3k budget, indoor
{
  ...
  test(
    "R",
    "R8 Racket specialist (low budget) → TT or Badminton in top 3",
...
// agility, so a faithful cricket archetype shouldn't inherit that default.
{
  ...
  test(
    "R",
    "R9 Cricket profile → Cricket in top 3",
    inTop3(a, "Cricket"),
    `got: ${top.join(", ")}`
  );
}
...
// R10: Volleyball — tall, explosive, team-oriented, either environment
{
  ...
  test(
    "R",
    "R10 Tall team-oriented profile (age 10) → Volleyball in top 3",
    inTop3(a, "Volleyball"),
    `got: ${top.join(", ")}`
  );
}
...
// E1: Water discomfort → Swimming absent even for endurance/repetition swimmer profile
{
  ...
  test(
    "E",
    "E1 Water discomfort → Swimming hard-gated",
    absent(a, "Swimming"),
    `top 3: ${top3names(a).join(", ")}`
  );
}
...
// E2: Budget under-3k → Tennis absent (minBudgetTier = 7k-15k)
{
  ...
  test(
    "E",
    "E2 Budget under-3k → Tennis hard-gated",
    absent(a, "Tennis"),
    `top 3: ${top3names(a).join(", ")}`
  );
}
...
// E3: 155cm boy, national ambition, age 14 → Volleyball absent (minH boy = 172)
{
  ...
  test(
    "E",
    "E3 155cm boy + national volleyball → height gate fires",
    absent(a, "Volleyball"),
    `top 3: ${top3names(a).join(", ")}`
  );
}
...
// E4: 156cm girl, national ambition, age 14 → Basketball absent (minH girl = 160)
{
  ...
  test(
    "E",
    "E4 156cm girl + national basketball → height gate fires",
    absent(a, "Basketball"),
    `top 3: ${top3names(a).join(", ")}`
  );
}
...
// (generic age-cutoff hard gate — applies to every sport, not just Swimming)
{
  ...
  test(
    "E",
    "E5 Age 16 + professional swimming → age cutoff gate fires (cutoff=13)",
    absent(a, "Swimming"),
    `top 3: ${top3names(a).join(", ")}`
  );
}
...
// for the empirical basis)
{
  ...
  test(
    "A",
    `A1 Perfect badminton profile → score ≥90 (got ${score})`,
    score >= 90,
    `Badminton ranked #${results.findIndex((r) => r.sport.name === "Badminton") + 1 || "outside top 3"}`
  );
}
...
// A2: Score spread — #1 and #3 should differ by ≥ 5 points
{
  ...
  if (results.length >= 3) {
    ...
    test(
      "A",
      `A2 Score spread: #1=${results[0].score} #3=${results[2].score} gap=${gap} (≥5)`,
      gap >= 5,
      `${results.map((r) => `${r.sport.name}:${r.score}`).join(", ")}`
    );
  } else {
...
//     With old dimMatch: dimMatch(5,1) = 0.0 would destroy Chess; capMatch = 1.0
{
  ...
  test(
    "A",
    "A3 capMatch: high-agility chess player → Chess not penalised",
    inTop3(a, "Chess"),
    `got: ${top.join(", ")}`
  );
}
...
//     priorSports includes same sport → +0.05 retake bonus should dominate ranking
{
  ...
  test(
    "A",
    `A4 Prior sport: retake bonus pushes Badminton to #1 when Badminton is prior (got: ${retakeTop})`,
    retakeTop === "Badminton",
    `priorSports=["Badminton"]; adjacent test: TT prior → ${adjacentTop} (TT retake +0.05 > adj +0.025)`
  );
}
...
//     hits the hard cutoff (14) — soft degradation, not a cliff.
{
  ...
  test(
    "A",
    `A5 Moderate age sensitivity: Badminton score drops past ideal window (age9=${score9} > age13=${score13})`,
    score9 > score13,
...
//     which is a badminton-flavoured default, not a cricket one).
{
  ...
  test(
    "A",
    `A6 Flexible age sensitivity: Cricket age 16, professional → score ≥70`,
    (cricket?.score ?? 0) >= 70,
...
// A7: Racket synergy fires — explosive + high agility → Badminton or TT in top 3
{
  ...
  test(
    "A",
    "A7 Racket synergy: explosive + agile → racket sport in top 3",
...
// A8: Ambition level changes rankings for the same profile (age matters more at national)
{
  ...
  test(
    "A",
    "A8 Ambition level affects rankings",
...
// A10: Reasons are generated and non-trivial (≥2 reasons, each ≥25 chars)
{
  const a: WizardAnswers = {
    ...
    eyesight: "sharp",
    visualTracking: "strong",
    priorSports: ["Table Tennis"],
  };
  const results = scoreSports(a);
  const top = results[0];
  const ok =
    top !== undefined && top.reasons.length >= 2 && top.reasons.every((r) => r.length >= 25);
  test(
    "A",
    `A10 Reasons: ${top?.reasons.length ?? 0} generated, all non-trivial`,
    ok,
    `First: "${top?.reasons[0]?.slice(0, 60)}…"`
  );
}
...
//     children who differ only on that one answer.
{
  ...
  const scoreOf = (a: WizardAnswers, sport: string) => scoreChosenSports(a, [sport])[0]?.score ?? 0;
  ...
  test(
    "A",
    `A11 Solo answer favours individual sports (Chess solo=${chessSolo} > team=${chessTeam})`,
    chessSolo > chessTeam,
    `An inverted axis flips this comparison`
  );
  test(
    "A",
    `A11b Team answer favours team sports (Football team=${footballTeam} > solo=${footballSolo})`,
...
//      Chess-shaped — endurance + deep focus + solo describes both.
{
  ...
  test(
    "A",
    `A12 Convergent swimmer persona → #1 Swimming, score ${result?.score ?? 0}`,
    ok,
    `got: ${result?.sport.name} (${result?.score})`
  );
}
...
function probe(label: string, a: WizardAnswers) {
  ...
  r.forEach((s, i) =>
    console.log(
      `    ${D}#${i + 1} ${s.sport.name.padEnd(14)} ${s.score.toString().padStart(3)}/100  ${s.fitLabel}${X}`
    )
  );
  console.log();
}
...
const byR = byCategory("R"),
  byE = byCategory("E"),
  byA = byCategory("A");
console.log(
  `  ${C}Relevance   ${X}  ${byR.filter((r) => r.ok).length}/${byR.length} tests   ${bar(relevance)} ${B}${relevance}/100${X}`
);
console.log(
  `  ${C}Efficiency  ${X}  ${byE.filter((r) => r.ok).length}/${byE.length} tests   ${bar(efficiency)} ${B}${efficiency}/100${X}`
);
console.log(
  `  ${C}Accuracy    ${X}  ${byA.filter((r) => r.ok).length}/${byA.length} tests   ${bar(accuracy)} ${B}${accuracy}/100${X}`
);
console.log(`  ${"─".repeat(50)}`);
const overallColor = overall >= 80 ? G : overall >= 60 ? Y : R;
console.log(
  `  ${B}Overall   ${overallColor}${overall}/100${X}  ${overall >= 80 ? `${G}Good` : overall >= 60 ? `${Y}Needs tuning` : `${R}Needs work`}${X}`
);
console.log(`${B}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${X}\n`);
