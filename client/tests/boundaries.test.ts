...
const BANNED: ReadonlyArray<[string, string, string]> = [
  ...
  ["features must not import routes", "src/modules/booking/services/x.ts", "@/app/(booking)/checkout/page"],
  ["the app shell must not import routes", "src/components/layout/x.tsx", "@/app/(booking)/checkout/page"],
  ...
  ["features must not import the app shell", "src/modules/booking/services/x.ts", "@/components/layout/Footer"],
  ...
  ["primitives must not import features", "src/modules/shared/ui/X.tsx", "@/modules/auth/store/authStore"],
  ["primitives must not import the app shell", "src/modules/shared/ui/X.tsx", "@/components/layout/Footer"],
  ...
  ["primitives must not import routes", "src/modules/shared/ui/X.tsx", "@/app/(booking)/checkout/page"],
];

const ALLOWED: ReadonlyArray<[string, string, string]> = [
  // Imports pointing *down* the layer list, which is the whole point.
  ["a feature may import another feature", "src/modules/booking/services/x.ts", "@/modules/auth/store/authStore"],
  ["a feature may import primitives", "src/modules/booking/services/x.ts", "@/modules/shared/ui/Button"],
  ["a feature may import infrastructure", "src/modules/booking/services/x.ts", "@/lib/query/keys"],
  ["the app shell may import features", "src/components/layout/x.tsx", "@/modules/auth/store/authStore"],
  ["a route may import anything", "src/app/(booking)/checkout/page.tsx", "@/components/layout/Footer"],
