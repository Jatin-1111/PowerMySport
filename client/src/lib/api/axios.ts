 ...
 */
export const returnPathForUnauthorized = (pathname: string, search?: string): string | null =>
...
axiosInstance.interceptors.response.use(
  ...
  (error: AxiosError) => {
    ...
    const errorMessage = ((error.response?.data as any)?.message as string | undefined) || "";
    ...
    if ((error.response?.status === 401 || isStaleProfileSession) && !isAuthEndpoint) {
      if (typeof window !== "undefined") {
        ...
        const returnTo = returnPathForUnauthorized(pathname, window.location.search);
