...
import { ArrowLeft, ChevronDown, Search, ShoppingCart, User } from "lucide-react";
    ...
    <header className="fixed top-4 left-1/2 z-50 w-[calc(100%-2rem)] max-w-7xl -translate-x-1/2 rounded-full border border-slate-200/70 bg-white/90 shadow-md backdrop-blur-xl transition-all duration-300">
      <div className="flex flex-col gap-3 px-4 py-2.5 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="group relative ml-4 hidden lg:block">
            {/* Added a taller trigger area (py-4) to create an invisible bridge to the dropdown menu */}
            <button className="flex items-center gap-2 py-4 text-sm font-bold text-slate-600 transition-colors group-hover:text-slate-900">
              Collections
