...
function CoachesTabContent() {
  ...
  const getDisplayName = (coach: Coach) => {
    ...
    return typeof name === "string" && name.trim()
      ? name.trim()
      : `${coach.sports[0] || "Professional"} Coach`;
  };
  ...
  const getServingCity = (coach: Coach) => {
    const parts = (coach.ownVenueDetails?.address || "")
      .split(",")
      .map((p) => p.trim())
      .filter(Boolean);
  ...
  const applyFilters = (base: Coach[]) => {
    ...
    if (sortBy === "priceAsc") {
      next = [...next].sort((a, b) => {
        const ra = getComparableRate(a);
        const rb = getComparableRate(b);
        if (ra === null) return 1;
        if (rb === null) return -1;
        return ra - rb;
      });
      ...
      next = [...next].sort((a, b) => {
        const da = getDistanceFromUser(a);
        const db = getDistanceFromUser(b);
        if (da === null && db === null)
          return getRelevanceScore(b, term) - getRelevanceScore(a, term);
        if (da === null) return 1;
        if (db === null) return -1;
        return da - db;
      });
  ...
  if (serviceModeFilter !== "ALL") {
    activeFilters.push({
      id: "mode",
      label:
        serviceModeFilter === "OWN_VENUE"
          ? "Own Venue"
          : serviceModeFilter === "FREELANCE"
            ? "Freelance"
            : "Hybrid",
      onRemove: () => setServiceModeFilter("ALL"),
      badgeClassName: "bg-emerald-50 border-emerald-100 text-turf-green",
      iconClassName: "hover:text-emerald-700",
    });
  }
  if (maxRate) {
    activeFilters.push({
      id: "rate",
      label: `Max ₹${maxRate}/hr`,
      onRemove: () => setMaxRate(""),
      badgeClassName: "bg-emerald-50 border-emerald-100 text-turf-green",
      iconClassName: "hover:text-emerald-700",
    });
  ...
  if (sortBy !== "relevance") {
    activeFilters.push({
      id: "sort",
      label: `Sort: ${sortBy === "priceAsc" ? "Price ↑" : sortBy === "priceDesc" ? "Price ↓" : sortBy === "nearest" ? "Nearest" : "Top Rated"}`,
  ...
  if (certifiedOnly) {
    activeFilters.push({
      id: "certified",
      label: "Certified",
      onRemove: () => setCertifiedOnly(false),
      badgeClassName: "bg-indigo-50 border-indigo-100 text-indigo-700",
      iconClassName: "hover:text-indigo-900",
    });
  }
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-slate-100 bg-white p-8 text-center">
            ...
            <h3 className="text-lg font-bold text-slate-900">
              {appliedSportFilter
                ? `No coaches for "${appliedSportFilter}"`
                ...
                <a
                  ...
                  onClick={() =>
                    statsApi.trackFunnelEventNonBlocking({
                      ...
                      entityType: "Coach",
                      metadata: communityIntent.analyticsMetadata,
                      source: "WEB",
                    })
                  }
                >
                    ...
                    >
                      ...
                      <div className="flex flex-1 flex-col p-5">
                        ...
                        <p className="mt-3 line-clamp-2 text-sm leading-relaxed text-slate-500">
                          {typeof coach.bio === "string" && coach.bio.trim()
                            ? coach.bio.trim()
