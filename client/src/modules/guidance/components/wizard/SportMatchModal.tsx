...
import { X, Sparkles, ChevronRight, Target, Wallet, Activity, MapPin } from "lucide-react";
import { Button } from "@/modules/shared/ui/Button";
import { PERSONALITY_OPTIONS, OBJECTIVES, BUDGET_OPTIONS, INDIAN_STATES } from "../../constants";
        ...
        >
          ...
          <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 bg-white/80 px-6 py-4 backdrop-blur-md">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-title text-lg font-bold text-slate-900">Sport Match Recommendation</h2>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="p-6">
            {step === "loading" && (
              <div className="flex flex-col items-center justify-center py-20">
                ...
                <p className="font-title text-lg font-bold text-slate-900">Finding the perfect match...</p>
                <p className="text-sm text-slate-500">Analyzing pathways, costs, and traits.</p>
              </div>
            )}

            {step === "select_dependent" && (
              <div className="mx-auto max-w-xl py-8 text-center">
                <h3 className="text-2xl font-bold text-slate-900 mb-6">Who are we finding a sport for?</h3>
                {localDependents && localDependents.length > 0 ? (
                  <div className="space-y-3">
                    {localDependents.map(dep => {
                      const calcAge = dep.age || (dep.dob ? Math.floor((new Date().getTime() - new Date(dep.dob).getTime()) / 31557600000) : null);
                      return (
                        <button
                          key={dep._id}
                          onClick={() => {
                        ...
                        >
                          ...
                          <div>
                            ...
                            <p className="text-sm text-slate-500">{calcAge ? `${calcAge} years old` : "Age not set"}</p>
                          </div>
            ...
            {step === "questions" && (
              <div className="mx-auto max-w-2xl py-6">
                  ...
                  <div className="flex items-center gap-2">
                    {[1, 2, 3].map(s => (
                      <div key={s} className="flex items-center gap-2">
                        <div className={`flex h-8 w-8 items-center justify-center rounded-full font-bold text-sm transition-all ${qStep === s ? 'bg-power-orange text-white ring-4 ring-orange-50' : qStep > s ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-400'}`}>
                          {s}
                        </div>
                        {s < 3 && <div className={`h-1 w-12 rounded-full ${qStep > s ? 'bg-slate-800' : 'bg-slate-100'}`} />}
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <h3 className="text-2xl font-bold text-slate-900 mb-6 text-center">
                  {qStep === 1 ? "Let's get to know the athlete" : qStep === 2 ? "Activity & Location" : "Goals & Preferences"}
                </h3>
                ...
                <div className="space-y-6">
                  {qStep === 1 && (
                    <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                          <label className="block text-sm font-bold text-slate-700 mb-2">Athlete Name</label>
                          <input 
                            type="text" 
                        ...
                        <div>
                          ...
                          <input 
                            ...
                            onChange={e => setManualData({...manualData, child_age: e.target.value})}
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                          ...
                          <select 
                            ...
                            onChange={e => setManualData({...manualData, gender: e.target.value as any})}
                          >
                        ...
                        <div>
                          <label className="block text-sm font-bold text-slate-700 mb-2">Relation to you</label>
                          <select 
                            className="w-full rounded-2xl border-slate-200 px-4 py-3 focus:border-power-orange focus:ring-power-orange bg-slate-50 focus:bg-white transition-all appearance-none"
                            value={manualData.relation}
                            onChange={e => setManualData({...manualData, relation: e.target.value})}
                          >
                  ...
                  {qStep === 2 && (
                    <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                          <label className="block text-sm font-bold text-slate-700 mb-2">State/Location</label>
                          <select 
                            className="w-full rounded-2xl border-slate-200 px-4 py-3 focus:border-power-orange focus:ring-power-orange bg-slate-50 focus:bg-white transition-all appearance-none"
                        ...
                        <div>
                          <label className="block text-sm font-bold text-slate-700 mb-2">Weekly Time Commitment (Hours)</label>
                          <input 
                            type="number" 
                            min="1" max="40"
                            className="w-full rounded-2xl border-slate-200 px-4 py-3 focus:border-power-orange focus:ring-power-orange bg-slate-50 focus:bg-white transition-all"
                            value={manualData.weeklyTimeCommitment}
                            onChange={e => setManualData({...manualData, weeklyTimeCommitment: parseInt(e.target.value) || 3})}
                          />
                        </div>
                      </div>

                      <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Sports Currently Playing (Optional)</label>
                        <SportsMultiSelect 
                      ...
                      {manualData.sports.length > 0 && (
                        <div className="animate-in fade-in zoom-in-95 duration-200">
                          <label className="block text-sm font-bold text-slate-700 mb-2">Years Playing</label>
                          <input 
                            type="number" 
                            min="0" max="20"
                            placeholder="e.g., 2"
                            ...
                            onChange={e => setManualData({...manualData, yearsPlaying: e.target.value})}
                          />
                        </div>
                      )}
                    </div>
                  ...
                  {qStep === 3 && (
                    <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                          {OBJECTIVES.map(obj => {
                            ...
                            return (
                              <button
                                key={obj.value}
                                onClick={() => setManualData({...manualData, primary_objective: obj.value})}
                              ...
                              >
                                <Icon className={`h-8 w-8 mb-3 ${isSelected ? 'text-power-orange' : 'text-slate-400'}`} />
                      ...
                      <div>
                        <label className="block text-sm font-bold text-slate-700 mb-3">Budget Tier</label>
                        <div className="grid grid-cols-3 gap-3">
                          {BUDGET_OPTIONS.map(b => (
                            <button
                              key={b.value}
                              onClick={() => setManualData({...manualData, budget_tier: b.value})}
                      ...
                      <div>
                        <label className="block text-sm font-bold text-slate-700 mb-3">Personality Traits (Select up to 3)</label>
                          ...
                          {PERSONALITY_OPTIONS.map(p => {
                            ...
                            return (
                              <button
                                ...
                                onClick={() => {
                                  if (isSelected) {
                                    setManualData({...manualData, personality_tags: manualData.personality_tags.filter(t => t !== p.label)});
                                  } else if (manualData.personality_tags.length < 3) {
                                    setManualData({...manualData, personality_tags: [...manualData.personality_tags, p.label]});
                                  }
                                }}
                  ...
                  <div className="flex justify-between pt-8 border-t border-slate-100 mt-8">
                    <Button 
                      variant="outline" 
                      onClick={() => qStep > 1 ? setQStep(qStep - 1) : setStep("select_dependent")}
                    >
                      Back
                    </Button>
                    ...
                    {qStep < 3 ? (
                      <Button 
                        variant="primary" 
                        disabled={(qStep === 1 && (!manualData.name || !manualData.child_age)) || (qStep === 2 && !manualData.location)}
                      ...
                      <Button 
                        variant="primary" 
                        disabled={!manualData.primary_objective || !manualData.budget_tier || manualData.personality_tags.length === 0}
            ...
            {step === "profile_summary" && selectedDependent && (
              <div className="mx-auto max-w-xl py-8">
                <h3 className="text-2xl font-bold text-slate-900 mb-2 text-center">Using {selectedDependent.name}'s Profile</h3>
                <p className="text-center text-slate-500 mb-8">We'll use these details to find the best sport.</p>
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6 mb-8">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-semibold text-slate-800">{selectedDependent.age || (selectedDependent.dob ? Math.floor((new Date().getTime() - new Date(selectedDependent.dob).getTime()) / 31557600000) : "Not specified")}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Location</p>
                      <p className="font-semibold text-slate-800">{selectedDependent.location || "Not specified"}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Objective</p>
                      <p className="font-semibold text-slate-800">{selectedDependent.primaryObjective || "Not specified"}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Budget</p>
                      <p className="font-semibold text-slate-800">{selectedDependent.budgetTier || "Not specified"}</p>
                    </div>
                  </div>
                  <div className="mt-4 pt-4 border-t border-slate-200">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Personality Tags</p>
                    <div className="flex flex-wrap gap-2">
                      {selectedDependent.personalityTags?.length ? selectedDependent.personalityTags.map(tag => (
                        <span key={tag} className="rounded-full bg-white border border-slate-200 px-3 py-1 text-[11px] font-semibold text-slate-600">
                          {tag}
                        </span>
