...
export function PaymentTypeSelector({
  ...
  const amountPerPerson = participantCount > 0 ? totalAmount / participantCount : totalAmount;
    ...
    <div className={cn("space-y-3", className)}>
      ...
      <button
        ...
        className={cn(
          ...
          value === "SINGLE"
            ...
            : "hover:border-power-orange/60 border-slate-200 bg-white"
        )}
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="font-semibold text-slate-900">I'll pay for everyone</p>
            <p className="mt-1 text-sm text-slate-600">
              You cover the full amount now. Perfect for treating your friends!
            </p>
            <div className="text-power-orange mt-2 flex items-center gap-1.5 text-sm font-semibold">
              <span>{formatCurrency(totalAmount)}</span>
              <span className="text-xs font-normal text-slate-500">(Full amount)</span>
            </div>
          </div>
        </div>
        <div
          className={cn(
            ...
            value === "SINGLE" ? "border-power-orange bg-power-orange" : "border-slate-300 bg-white"
          )}
        >
          {value === "SINGLE" && (
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="font-semibold text-slate-900">Split payment equally</p>
            <p className="mt-1 text-sm text-slate-600">
        ...
        <div
          className={cn(
            "mt-1 h-5 w-5 shrink-0 rounded-full border-2 transition-all",
            value === "SPLIT" ? "border-power-orange bg-power-orange" : "border-slate-300 bg-white"
          )}
        >
          {value === "SPLIT" && <div className="h-full w-full scale-[0.4] rounded-full bg-white" />}
        </div>
      </button>

      {/* Info Note */}
      {value === "SPLIT" && (
        <div className="rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-3 text-sm text-blue-800">
          <p className="font-medium">How split payment works:</p>
          <ul className="mt-2 space-y-1 text-xs">
            <li>• Each participant pays their share separately</li>
            <li>• Booking confirmed once payments are complete and the provider approves</li>
