...
export const pathwayProfileApi = {
  getProfile: async (): Promise<UserPathwayProfile | null> => {
    try {
      const resp = await axiosInstance.get<{ success: boolean; data: UserPathwayProfile }>("/pathway-profile");
  ...
  updateProfile: async (data: Partial<UserPathwayProfile>): Promise<UserPathwayProfile | null> => {
    try {
      const resp = await axiosInstance.put<{ success: boolean; data: UserPathwayProfile }>("/pathway-profile", data);
      return resp.data.data;
    } catch {
      return null;
    }
