...
export default function CoachDetailsPage() {
  ...
  useEffect(() => {
    if (coachId) loadCoachDetails();
  }, [coachId, loadCoachDetails]);
  useEffect(() => {
    if (coachId && selectedDate) loadAvailability();
  }, [coachId, selectedDate, selectedSport, loadAvailability]);
  useEffect(() => {
    setSelectedSlot(null);
  }, [selectedSport, selectedDate]);
  useEffect(() => {
    if (coachId) loadReviews();
  }, [coachId]);
  useEffect(() => {
    if (coachId && user?.id) loadReviewEligibility();
    else {
  ...
  useEffect(() => {
    if (coachId) void loadSubscriptionPackages();
                ...
                {user && eligibleBookingId && (
                  <div className="mb-6 rounded-xl border border-slate-200 bg-slate-50/90 p-4">
                    ...
                    <div className="mt-3 flex justify-end">
                      <Button
                        variant="primary"
                        className="bg-turf-green hover:bg-emerald-700"
                          ...
                          return (
                            ...
                            >
                              <div className="flex flex-col items-center justify-center">
                                ...
                                {!isSlotAvailable && (
                                  <span
                                    className={`text-[10px] font-semibold ${isSelected ? "text-white/90" : "text-amber-600"}`}
                                  >
                                    Join Waitlist
                                  </span>
                                )}
