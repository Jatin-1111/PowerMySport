...
export default function InvitationsPage() {
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs items={[{ label: "Dashboard", href: "/dashboard" }, { label: "Invitations" }]} />
            ...
            <CardContent className="space-y-4 px-6 py-5">
              {pendingInvitations.length === 0 ? (
                ...
                pendingInvitations.map((invitation) => (
                  ...
                  >
                    {/* Accent stripe */}
                    <div className="bg-power-orange w-1 shrink-0" />
                    <div className="flex flex-1 flex-col gap-4 bg-slate-50/40 p-4 sm:p-5">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <Badge className="text-power-orange shrink-0 border-orange-200 bg-orange-50 capitalize hover:bg-orange-50">
                          {invitation.sport}
                        </Badge>
                      </div>
                      ...
                      <div className="grid gap-2 text-sm">
                        ...
                        {invitation.estimatedAmount > 0 && (
                          <div className="mt-1 inline-flex items-center gap-1.5 self-start rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-sm font-semibold text-emerald-700">
                      ...
                      <div className="flex gap-3 pt-1">
                        ...
                        <Button
                          ...
                          className="flex-1 sm:flex-none"
                          onClick={() =>
                            handleRespond(invitation.id || invitation._id || "", false)
                          }
            ...
            <Card className="shop-surface premium-shadow overflow-hidden p-0">
              ...
              <CardContent className="space-y-3 px-6 py-5">
                {respondedInvitations.map((invitation) => (
                  <div
                    key={invitation.id || invitation._id}
                    className="flex overflow-hidden rounded-xl border border-slate-200/70 opacity-75 transition-opacity hover:opacity-100"
                  >
                    <div
                      className={`w-1 shrink-0 ${
                        invitation.status === "ACCEPTED" ? "bg-turf-green" : "bg-slate-300"
                      }`}
                    />
                    <div className="flex flex-1 items-center justify-between gap-3 bg-slate-50/30 p-4">
                      <div>
                        <p className="font-semibold text-slate-900">{invitation.venueId.name}</p>
                      ...
                      <Badge
                        className={
                          invitation.status === "ACCEPTED"
                            ...
                            : "shrink-0 border-slate-200 bg-slate-100 text-slate-600 hover:bg-slate-100"
                        }
                      >
                        {invitation.status}
                      </Badge>
