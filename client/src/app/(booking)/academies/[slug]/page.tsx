...
type TabKey = "overview" | "coaches" | "venues" | "subscriptions" | "packages" | "gallery" | "reviews";
...
export default function AcademyProfilePage() {
  ...
  useEffect(() => {
    ...
    if (slug) void loadAcademyProfile();
  }, [slug]);
    ...
    return (
      <div className="min-h-screen bg-[linear-gradient(180deg,#eef4ff_0%,#f4f8ff_46%,#fff8ee_100%)] flex flex-col items-center justify-center">
        <Card className="w-full max-w-md rounded-2xl border-slate-200 bg-white/90 p-8 text-center">
          ...
          <p className="mt-2 text-sm text-slate-600">This academy profile may be unavailable or not publicly visible yet.</p>
          <div className="mt-5">
  ...
  const handleBookSession = () => {
    const checkoutParams = new URLSearchParams({ type: "academy" });
    if (academyMongoId) checkoutParams.set("academyId", academyMongoId);
    checkoutParams.set("academySlug", encodeURIComponent(slug));
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#eef4ff_0%,#f4f8ff_46%,#fff8ee_100%)]">
      <section className="mx-auto w-full max-w-7xl px-4 pt-4 pb-8 sm:px-6 lg:px-8">
        <div className="mb-4">
          <Link href="/booking?tab=academies" className="inline-flex items-center gap-1 text-sm font-semibold text-power-orange hover:text-orange-700">
            ← Back to academies
          </Link>
        </div>
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white/90 shadow-sm">
          <div className="relative h-60 w-full bg-slate-200 sm:h-72 lg:h-80">
            {coverImage ? (
              ...
              <div className="bg-linear-to-br from-slate-200 to-slate-300 flex h-full items-center justify-center text-slate-600">No cover photo</div>
            )}
          </div>
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="flex items-start gap-4">
                <div className="h-16 w-16 overflow-hidden rounded-2xl border border-slate-200 bg-white sm:h-20 sm:w-20">
                  {logoImage ? (
                    <img src={logoImage} alt={`${academy.name} logo`} className="h-full w-full object-cover" />
                  ) : (
                    <div className="flex h-full w-full items-center justify-center bg-slate-100 text-xs font-semibold text-slate-500">Logo</div>
                  )}
                </div>
                ...
                <div>
                  ...
                  <p className="inline-flex items-center gap-1 text-sm text-slate-600">
                    <MapPin size={14} />
                    {[academy.city, academy.state].filter(Boolean).join(", ") || "Location unavailable"}
                  </p>
                  <div className="mt-3 flex flex-wrap items-center gap-3">
                    ...
                    <p className="text-sm font-semibold text-slate-700">
                      ...
                      <span className="ml-1 font-normal text-slate-500">({academy.reviewCount || 0} reviews)</span>
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {(academy.sports || []).map((sport) => (
                  <span key={sport} className="rounded-full border border-orange-200 bg-orange-50 px-3 py-1 text-xs font-semibold text-orange-700">{sport}</span>
                ))}
              </div>
            </div>
          </div>
          ...
          {activeTab === "overview" && (
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              <Card className="rounded-2xl border-slate-200 bg-white/90 lg:col-span-2">
                ...
                <p className="mt-3 text-sm leading-relaxed text-slate-700">{academy.description || "No description provided yet."}</p>
                <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Age Groups</p>
                    <div className="mt-2 flex flex-wrap gap-2">
                      {(academy.ageGroups || []).length > 0 ? (academy.ageGroups || []).map((group) => (
                        <span key={group} className="rounded-full border border-slate-200 bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">{formatAgeGroup(group)}</span>
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-sm text-slate-700">{(academy.languagesSpoken || []).join(", ") || "Not provided"}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Established</p>
                    <p className="mt-2 text-sm text-slate-700">{academy.establishedYear || "N/A"}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Batch Size</p>
                    <p className="mt-2 text-sm text-slate-700">{academy.maxBatchSize ? `${academy.maxBatchSize} students` : "Not provided"}</p>
                  </div>
                </div>
              </Card>
              ...
              <Card className="rounded-2xl border-slate-200 bg-white/90">
                ...
                <div className="mt-4 space-y-3 text-sm text-slate-700">
                  <p className="inline-flex items-center gap-2"><MapPin size={14} className="text-slate-500" />{[academy.address, academy.city, academy.state, academy.pincode].filter(Boolean).join(", ") || "Address unavailable"}</p>
                  <p className="inline-flex items-center gap-2"><Users size={14} className="text-slate-500" />{academy.contactPersonName || "Contact person not provided"}</p>
                  <p className="inline-flex items-center gap-2"><Mail size={14} className="text-slate-500" />{academy.contactEmail || "Email unavailable"}</p>
                  <p className="inline-flex items-center gap-2"><Phone size={14} className="text-slate-500" />{academy.contactPhone || academy.whatsappNumber || "Phone unavailable"}</p>
                </div>
                {academy.placeId && (
                  <div className="mt-4 overflow-hidden rounded-xl border border-slate-200">
                    <iframe title="Academy map" src={`https://www.google.com/maps?q=place_id:${academy.placeId}&output=embed`} loading="lazy" className="h-56 w-full" />
                  </div>
                )}
              </Card>

              <Card className="rounded-2xl border-slate-200 bg-white/90 lg:col-span-3">
                <h2 className="inline-flex items-center gap-2 text-lg font-bold text-slate-900"><Clock3 size={18} />Operating Hours</h2>
                <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-4">
                  {dayOrder.map((day) => {
                    const dayConfig = academy.operatingHours?.[day];
                    const label = day.charAt(0).toUpperCase() + day.slice(1);
                    return (
                      <div key={day} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                        <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{label}</p>
                        <p className="mt-1 text-sm font-medium text-slate-700">{dayConfig?.isOpen ? `${dayConfig.openTime || "--:--"} - ${dayConfig.closeTime || "--:--"}` : "Closed"}</p>
                      </div>
                    );
                  })}
                </div>
              ...
              {academy.coachIds && academy.coachIds.length > 0 ? (
                <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {academy.coachIds.map((coach, index) => {
                    const coachObj = typeof coach === "object" ? (coach as { _id?: string; id?: string; name?: string }) : null;
                    const key = coachObj?._id || coachObj?.id || `${index}`;
                    return (
                      <div key={key} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <p className="text-sm font-semibold text-slate-800">{coachObj?.name || `Coach ${index + 1}`}</p>
              ...
              {academy.venueIds && academy.venueIds.length > 0 ? (
                <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {academy.venueIds.map((venue, index) => {
                    const venueObj = typeof venue === "object" ? (venue as { _id?: string; id?: string; name?: string }) : null;
                    const key = venueObj?._id || venueObj?.id || `${index}`;
                    return (
                      <div key={key} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <p className="text-sm font-semibold text-slate-800">{venueObj?.name || `Venue ${index + 1}`}</p>
              ...
              {academy.subscriptionPlans && academy.subscriptionPlans.length > 0 ? (
                <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {academy.subscriptionPlans.map((plan, index) => {
                    const planObj = typeof plan === "object" ? (plan as { _id?: string; id?: string; name?: string; price?: number }) : null;
                    const key = planObj?._id || planObj?.id || `${index}`;
                    const price = typeof planObj?.price === "number" ? Math.round(planObj.price / 100) : null;
                    return (
                      <div key={key} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <p className="text-sm font-semibold text-slate-800">{planObj?.name || `Plan ${index + 1}`}</p>
                        <p className="mt-1 text-xs text-slate-600">{typeof price === "number" ? `₹${price}` : "Price available on request"}</p>
                      </div>
                    );
                  })}
                </div>
              ...
              {academy.sessionPackages && academy.sessionPackages.length > 0 ? (
                <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {academy.sessionPackages.map((pkg, index) => {
                    const packageObj = typeof pkg === "object" ? (pkg as { _id?: string; id?: string; name?: string; price?: number }) : null;
                    const key = packageObj?._id || packageObj?.id || `${index}`;
                    const price = typeof packageObj?.price === "number" ? Math.round(packageObj.price / 100) : null;
                    return (
                      <div key={key} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <p className="text-sm font-semibold text-slate-800">{packageObj?.name || `Package ${index + 1}`}</p>
                        <p className="mt-1 text-xs text-slate-600">{typeof price === "number" ? `₹${price}` : "Price available on request"}</p>
                      </div>
                    );
                  })}
              ...
              {galleryImages.length > 0 ? (
                <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {galleryImages.map((image, index) => (
                    <div key={`${image}-${index}`} className="h-56 overflow-hidden rounded-xl border border-slate-200 bg-slate-100">
                      <img src={image} alt={`${academy.name} gallery ${index + 1}`} className="h-full w-full object-cover" />
                    </div>
                  ))}
                </div>
            ...
            <Card className="rounded-2xl border-slate-200 bg-white/90">
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  <p className="mt-1 text-2xl font-bold text-slate-900">{typeof academy.rating === "number" ? academy.rating.toFixed(1) : "0.0"}</p>
                </div>
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Total Reviews</p>
                  <p className="mt-1 text-2xl font-bold text-slate-900">{academy.reviewCount || 0}</p>
                </div>
              </div>
              <p className="mt-5 text-sm text-slate-600">Detailed review feed will be connected in the reviews module integration.</p>
            </Card>
          )}
        </div>
        ...
        <section className="mt-8">
          <Card className="rounded-2xl border-slate-200 bg-white/90">
            <h2 className="inline-flex items-center gap-2 text-lg font-bold text-slate-900"><CalendarDays size={18} />Ready to train?</h2>
            <p className="mt-2 text-sm text-slate-600">Book a session directly or contact the academy to discuss batches, schedules, and trials.</p>
            <div className="mt-4 flex flex-wrap gap-3">
              <Button variant="primary" onClick={handleBookSession}>Book a Session</Button>
              {academy.contactPhone && (
                <a href={`tel:${academy.contactPhone}`}><Button variant="outline">Call Academy</Button></a>
              )}
              {academy.contactEmail && (
                <a href={`mailto:${academy.contactEmail}`}><Button variant="outline">Email Academy</Button></a>
              )}
            </div>
          </Card>
        </section>
