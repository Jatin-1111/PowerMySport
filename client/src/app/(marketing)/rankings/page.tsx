...
import { COMBO_GROUPS, comboHref, comboLabel } from "@/modules/rankings/config";
import { ArrowRight, ExternalLink } from "lucide-react";
import type { Metadata } from "next";
import Link from "next/link";

export const metadata: Metadata = {
  title: "AITA Tennis Rankings — Search by State, Age Group & Player | PowerMySport",
  description:
    "Official All India Tennis Association rankings, made searchable. Filter Boys and Girls Under-12 to Under-18, Men's and Women's Singles and Doubles by state, and follow a player's rank week by week.",
  alternates: { canonical: "/rankings" },
  openGraph: {
    title: "AITA Tennis Rankings — searchable, by state and age group",
    description:
      "Every AITA ranking list, filterable by state and searchable by name, with each player's week-by-week history.",
...
export default async function RankingsHubPage() {
  const meta = await fetchRankingMeta();
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <header className="max-w-3xl">
        <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">
          AITA Tennis Rankings
        </h1>
        <p className="mt-3 text-base text-muted-foreground sm:text-lg">
          The official All India Tennis Association lists, made searchable. Filter by
          state, look up a player by name or registration number, and see how a
          ranking has moved week by week — something the source PDFs cannot show you.
        </p>
      </header>
      ...
      <footer className="mt-12 rounded-lg border bg-muted/40 p-4 text-sm text-muted-foreground">
        <p>
          ...
          <span className="font-medium text-foreground">
            {meta?.source.federation ?? "All India Tennis Association"}
          </span>{" "}
          and mirrored here for search and history. PowerMySport is not affiliated
          with AITA. Always treat the official list as authoritative.
        </p>
        <a
          href={meta?.source.url ?? "https://aitatennis.com/playerranking/"}
          target="_blank"
          rel="noopener noreferrer nofollow"
          className="mt-2 inline-flex items-center gap-1.5 font-medium text-power-orange hover:underline"
        >
          View the official AITA rankings page
