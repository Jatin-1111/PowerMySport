...
function GuidancePageInner() {
  ...
  const alreadyAtLevel =
    isLevelPlan && form.current_pathway_level === initialLevel;
        ...
        <div className="pb-3">
          ...
          <h1 className="font-title text-xl font-bold leading-tight tracking-tight sm:text-2xl">
            {isLevelPlan ? (
              alreadyAtLevel ? (
                <>
                  Your {initialLevelLabel} level{" "}
                  <span className="text-power-orange">progress plan.</span>
                </>
              ) : (
                <>
                  Your {initialLevelLabel} level{" "}
                  <span className="text-power-orange">decision guide.</span>
                </>
              )
            ) : (
              <>
                        ...
                        {!isFormValid(form) && (
                          <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5">
                            <p className="text-[10px] font-bold uppercase tracking-wider text-amber-700 mb-1.5 flex items-center gap-1">
                              <Info className="h-3 w-3" /> Required before
                              generating
                            </p>
