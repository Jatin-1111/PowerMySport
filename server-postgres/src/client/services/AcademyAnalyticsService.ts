...
export const getAcademyEarnings = async (
  ...
  const thisMonth = {
    total: thisMonthB.reduce((s: number, b: any) => s + getAmount(b), 0),
    sessions: thisMonthB.length,
  };
  const lastMonth = {
    total: lastMonthB.reduce((s: number, b: any) => s + getAmount(b), 0),
    sessions: lastMonthB.length,
  };
  const pending = {
    total: pendingBookings.reduce((s: number, b: any) => s + getAmount(b), 0),
  ...
  completedBookings.forEach((b: any) => {
    ...
    const e = sportMap.get(s) ?? { total: 0, sessions: 0 };
    e.total += getAmount(b);
    e.sessions += 1;
    sportMap.set(s, e);
  });
...
export const getAcademyAnalytics = async (
  ...
  const empty: AcademyAnalyticsData = {
  ...
  return {
    overview: {
      ...
      returningStudents: returningSet.size,
      retentionRate,
      totalVenues: venueIds.length,
      totalCoaches: coachIds.length,
    },
    ...
    popularHours,
    studentRetention: {
      newStudents: studentIds.length - returningSet.size,
      returningStudents: returningSet.size,
    },
  };
};
