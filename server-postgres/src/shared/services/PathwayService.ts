...
function buildPersonalNotesPrompt(
  ...
  const childFacts = [
    ...
    persona.ambition
      ? `- The family's stated goal: ${AMBITION_LABEL[persona.ambition]}.`
      : null,
    persona.budgetRange
      ? `- The family's monthly sports budget: ${BUDGET_LABEL[persona.budgetRange]}.`
      : null,
    persona.weeklyHours
      ? `- Current training time: ${HOURS_LABEL[persona.weeklyHours]}.`
      : null,
  ]
    .filter(Boolean)
    .join("\n");
...
export class PathwayService {
  ...
  async getOrGeneratePersonalNotes(
    ...
    const cached = await prisma.pathwayPersonalNote.findUnique({
      where: { cacheKey },
    });
    const cachedNotes =
      (cached?.notes as unknown as PersonalNote[] | undefined) ?? [];
    ...
    const pathway = await prisma.sportPathway.findFirst({
      where: { cacheKey: pathwayKey },
    });
    const levels = ((pathway?.levels as any[]) ?? []) as any[];
    ...
    try {
      ...
      const prompt = buildPersonalNotesPrompt(
        (pathway as any)?.sportName ?? sportName,
        state,
        levels,
        persona,
      );
      ...
      await prisma.pathwayPersonalNote.upsert({
        where: { cacheKey },
        update: {
          sportSlug: slug,
          state,
