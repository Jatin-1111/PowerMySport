...
export const getAcademyEarningsHandler = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: error instanceof Error ? error.message : "Failed to fetch academy earnings",
    });
  }
};
...
export const getAcademyAnalyticsHandler = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: error instanceof Error ? error.message : "Failed to fetch academy analytics",
    });
  }
};
