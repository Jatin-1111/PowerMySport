...
export const getVenueEarningsHandler = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: error instanceof Error ? error.message : "Failed to fetch venue earnings",
    });
  }
};
...
export const getVenueAnalyticsHandler = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: error instanceof Error ? error.message : "Failed to fetch venue analytics",
    });
  }
};
