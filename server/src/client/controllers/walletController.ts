...
export const topUpWallet = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(400).json({
      success: false,
      message: error instanceof Error ? error.message : "Failed to initiate top-up",
    });
  }
};
...
export const verifyTopUp = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!merchantOrderId) {
      res.status(400).json({ success: false, message: "Missing merchantOrderId" });
      return;
    }
    ...
    res.status(400).json({
      success: false,
      message: error instanceof Error ? error.message : "Failed to verify top-up",
    });
  }
};
