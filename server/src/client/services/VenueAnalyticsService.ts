...
export const getVenueEarnings = async (ownerUserId: string): Promise<VenueEarningsData> => {
  ...
  const [completedBookings, pendingBookings, recentBookings] = await Promise.all([
    ...
    Booking.find({ venueId: { $in: venueIds }, status: { $in: ["CONFIRMED", "IN_PROGRESS"] } }).lean(),
    Booking.find({ venueId: { $in: venueIds }, status: "COMPLETED" })
      .sort({ date: -1 }).limit(10)
      .populate("userId", "name photoUrl").lean(),
  ]);
  ...
  const allTime = { total: completedBookings.reduce((s: number, b: any) => s + getVenueAmount(b), 0), sessions: completedBookings.length };
  ...
  const thisMonth = { total: thisMonthB.reduce((s: number, b: any) => s + getVenueAmount(b), 0), sessions: thisMonthB.length };
  const lastMonth = { total: lastMonthB.reduce((s: number, b: any) => s + getVenueAmount(b), 0), sessions: lastMonthB.length };
  const pending = { total: pendingBookings.reduce((s: number, b: any) => s + getVenueAmount(b), 0), sessions: pendingBookings.length };
  ...
  completedBookings.filter((b: any) => new Date(b.date) >= sixMonthsAgo).forEach((b: any) => {
    ...
    if (e) { e.total += getVenueAmount(b); e.sessions += 1; }
  });
  ...
  completedBookings.forEach((b: any) => {
    ...
    e.total += getVenueAmount(b); e.sessions += 1;
...
export interface VenueAnalyticsData {
  overview: { totalSessions: number; completedSessions: number; completionRate: number; totalCustomers: number; returningCustomers: number; retentionRate: number; avgRating: number; reviewCount: number; };
...
export const getVenueAnalytics = async (ownerUserId: string): Promise<VenueAnalyticsData> => {
  ...
  if (venues.length === 0) {
    return {
      overview: { totalSessions: 0, completedSessions: 0, completionRate: 0, totalCustomers: 0, returningCustomers: 0, retentionRate: 0, avgRating: 0, reviewCount: 0 },
      sessionsTrend: [], sportBreakdown: [], popularHours: [], customerRetention: { newCustomers: 0, returningCustomers: 0 },
    };
  }
  const venueIds = venues.map((v: any) => v._id);
  ...
  const [allBookings, recentBookings] = await Promise.all([
    Booking.find({ venueId: { $in: venueIds }, status: { $nin: ["CANCELLED"] } }).select("userId sport date startTime status").lean(),
    Booking.find({ venueId: { $in: venueIds }, status: { $nin: ["CANCELLED"] }, date: { $gte: thirtyDaysAgo } }).select("date status").lean(),
  ]);
  ...
  countMap.forEach((c, id) => { if (c > 1) returningSet.add(id); });
  const retentionRate = allCustomerIds.length > 0 ? Math.round((returningSet.size / allCustomerIds.length) * 100) : 0;
  ...
  return {
    overview: { totalSessions: allBookings.length, completedSessions: completed.length, completionRate, totalCustomers: allCustomerIds.length, returningCustomers: returningSet.size, retentionRate, avgRating: Math.round(avgRating * 10) / 10, reviewCount },
    sessionsTrend, sportBreakdown, popularHours,
    customerRetention: { newCustomers: allCustomerIds.length - returningSet.size, returningCustomers: returningSet.size },
  };
};
