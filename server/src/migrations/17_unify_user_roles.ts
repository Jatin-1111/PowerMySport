...
export const up = async () => {
  ...
  res = await User.updateMany(
    { role: "VENUE_LISTER" },
    { $set: { role: "VenueLister" } },
  );
  console.log(
    `Updated ${res.modifiedCount} users from VENUE_LISTER to VenueLister.`,
  );
  ...
  console.log(
    `Updated ${res.modifiedCount} users from ACADEMY_OWNER to Academy.`,
  );
...
export const down = async () => {
  ...
  res = await User.updateMany(
    { role: "VenueLister" },
    { $set: { role: "VENUE_LISTER" } },
  );
  console.log(
    `Reverted ${res.modifiedCount} users from VenueLister to VENUE_LISTER.`,
  );
  ...
  console.log(
    `Reverted ${res.modifiedCount} users from Academy to ACADEMY_OWNER.`,
  );
