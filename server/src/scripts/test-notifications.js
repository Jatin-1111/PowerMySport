...
async function runTests() {
  ...
  try {
    ...
    const friendRequest = await sendFriendRequest(user1Token, user2Profile._id || user2Profile.id);
    ...
    console.log(
      `✅ User 2 has ${notifications.notifications?.length || notifications.length} notification(s)`
    );
    ...
    if (notifications.notifications?.length > 0 || notifications.length > 0) {
      ...
      const friendRequestNotification = notificationList.find((n) => n.type === "FRIEND_REQUEST");
