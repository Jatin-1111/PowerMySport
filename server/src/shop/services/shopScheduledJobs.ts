...
const log = __rootLog.child("shopScheduledJobs");
  ...
  try {
    ...
    for (const entry of pendingEntries) {
      try {
        ...
        log.info(`[Shop Waitlist] Sent launch email to: ${entry.email}`);
    ...
    if (notifiedCount > 0) {
      log.info(`Processed ${notifiedCount} waitlist notifications.`);
    }
  } catch (error) {
    log.error("Error processing waitlist notifications:", error);
  }
};
