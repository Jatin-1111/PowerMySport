...
export const normalizeAddressInput = <T extends NormalizableAddress>(data: T): T => {
  ...
  if (typeof out.addressLine1 === "string") out.addressLine1 = collapse(out.addressLine1);
  if (typeof out.addressLine2 === "string") out.addressLine2 = collapse(out.addressLine2);
  if (typeof out.city === "string") {
    const city = collapse(out.city);
    out.city = city ? toTitleCase(city) : city;
  }
  if (out.state !== undefined && out.state) out.state = normalizeStateName(out.state);
