...
function buildLevelRangePrompt(
  ...
  "governingBody": "Relevant governing body, ranking authority, or organizing body for this level in India",
  "localResources": {
...
- If a level is 2-5, the "label" MUST be a short (2-4 word) sport-specific, plain-English name for what that stage is actually called in "${sportName}". ${getLevelScopeGuidance(sportName)}
  ...
  private async generatePathway(
    ...
    const callModelWithFallback = async (
      ...
      if (isMeta) {
        ...
        const fallbackLabels = getLevelLabelFallbacks(sportName);
        return levelNums!.map((levelNum) => ({
          level: levelNum,
          label:
            levelNum === 1 ? "Beginner" : fallbackLabels[levelNum - 1],
          title: "Pathway Level",
    ...
    const levels = flatLevels.map((lvl: any, index: number) => {
      return {
        ...
        label:
          index === 0
            ? "Beginner"
            : lvl.label?.trim() || fallbackLabels[index],
      };
    });
