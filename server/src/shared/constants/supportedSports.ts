...
export const SUPPORTED_SPORT_SLUGS = new Set(SUPPORTED_SPORTS.map((s) => s.slug));

export const SUPPORTED_SPORT_NAMES = new Set(SUPPORTED_SPORTS.map((s) => s.name.toLowerCase()));
...
export function isSupportedSport(sportName: string): boolean {
  ...
  return (
    SUPPORTED_SPORT_SLUGS.has(slug) || SUPPORTED_SPORT_NAMES.has(sportName.trim().toLowerCase())
  );
}
