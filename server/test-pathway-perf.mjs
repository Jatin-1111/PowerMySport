...
async function main() {
  ...
  for (let i = 1; i <= RUNS; i++) {
    ...
    if (r.ok) {
      ...
      console.log(
        `SKIPPED — cacheKey lock already held (possibly stuck from a previous crashed run)`
      );
    } else if (r.status === "client_timeout") {
      console.log(
        `CLIENT TIMEOUT after ${CLIENT_TIMEOUT_MS}ms — server may still be working in the background`
      );
    } else {
  ...
  await fs.writeFile(OUT_PATH, JSON.stringify({ summary, runs: results }, null, 2));
