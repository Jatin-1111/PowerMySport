...
export default function CreateCommunityModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let finalProfilePictureKey = "";

      if (selectedFile) {
        const { uploadUrl, downloadUrl, key } = await communityService.getGroupImageUploadUrl(
        ...
        finalProfilePicture = downloadUrl;
                      ...
                      {profilePictureUrl ? (
                        ...
                        <div className="from-power-orange/20 to-power-orange/5 text-power-orange flex h-full w-full items-center justify-center bg-gradient-to-br text-3xl font-bold">
                          {name ? (
                            ...
                            <Users size={32} className="text-power-orange/40" />
                          )}
                        </div>
                      )}
                    </div>
